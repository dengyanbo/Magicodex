...
try {
    ...
    if (($hostInfo | Select-String '^host:').Line -match 'x86_64-pc-windows-gnu') {
        ...
        $env:CARGO_ENCODED_RUSTFLAGS = (@($flags) + @('-C', 'link-self-contained=yes', '-C', 'linker-flavor=ld.lld', '-C', "linker=$linker")) -join [char]0x1f
    }
    $config = @('--config', 'http.multiplexing=false', '--config', 'http.timeout=30')
