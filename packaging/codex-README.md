...
蓄力时法阵两侧是一本魔导书：左页“咏唱记录”把命令执行、文件修改、MCP、网页搜索与子代理等工具调用写成法术，右页“施法状态”显示咏唱时长、施法阶段与计数；两根与法阵同款的法阵柱随层数点亮，符文粒子向阵心汇聚，小猫使魔跟着吟唱和跑腿。终端 65 列起显示法阵柱与粒子，89 列起显示两页，109 列起显示参数与使魔；定格到对话记录里的出口不带两侧内容。
