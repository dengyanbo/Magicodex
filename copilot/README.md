...
- Copilot 启动、界面还没出现时，空白的屏幕中央会展开一座召唤法阵，“召唤 GitHub Copilot CLI”绕着它旋转，等得越久画得越完整；Copilot 一出现，它就化为光尘散去；
...
- 召唤法阵画在 Copilot 还空着的屏幕上，不改变 Copilot 的窗口大小（它启动时拿到的就是之后的大小）。Copilot 画出第一个字时召唤结束，法阵在 1.6 秒内散成光尘；光尘只落在离 Copilot 的文字至少 2 列的空白处，不会盖住文字，也不会落进词与词之间的空格。`--magic-no-motion` 时没有召唤法阵，启动时只显示静止的待机小阵；`--magic-off` 时法阵整个隐藏。
