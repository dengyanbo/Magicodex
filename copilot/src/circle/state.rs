...
/// Drops escape sequences and control characters other than whitespace.
fn sanitize_user_text(text: &str) -> String {
    let mut clean = String::with_capacity(text.len());
    let mut chars = text.chars().peekable();
    while let Some(c) = chars.next() {
        if c == '\u{1b}' {
            // Skip a CSI/OSC-style sequence up to its final byte or string terminator.
            match chars.peek() {
                Some('[') => {
                    chars.next();
...
impl MagicView<'_> {
    pub(crate) fn render_at(&self, area: Rect, buf: &mut Buffer, now: Instant) {
        ...
        match self.circle.started {
            None => styles::idle(self.style, &mut canvas, radius, &palette),
            Some(started) => {
                ...
                styles::draw(self.style, &mut canvas, &frame);
                if outlet {
                    let end = f64::from(height * 4) - 1.0 - center_y;
                    styles::outlet(self.style, &mut canvas, radius, end);
                }
            }
        }
